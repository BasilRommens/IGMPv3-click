...
InterfaceRecord::InterfaceRecord(in_addr interface, in_addr multicast_address, int filter_mode,
...
InterfaceMulticastTable::splitIncludeExclude(Vector<SocketRecord *>
                                             records) {
...
bool InterfaceMulticastTable::can_forward(in_addr src, in_addr multicast_addr) {
    if (is_ex(multicast_addr)) {
        return not is_in_source_list(src, multicast_addr);
    } else if (is_in(multicast_addr)) {
        return is_in_source_list(src, multicast_addr);
    }
    return false;
}

bool InterfaceMulticastTable::is_in_source_list(in_addr src, in_addr multicast_addr) {
    InterfaceRecord* record = getRecord(multicast_addr);
    // If there is no record don't bother searching for it, we will default assume there
    if (!record) {
        return false;
    }
    for (auto record_src: record->source_list) {
        if (src == record_src) {
            return true;
        }
    }
